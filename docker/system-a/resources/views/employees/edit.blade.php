...
<div class="card">
    <div class="card-header">
        <h2>Editar Empleado: {{ $employee->name }}</h2>
    </div>
    <div class="card-body">
                    ...
                    <select class="form-select @error('departments_id') is-invalid @enderror" 
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" 
                                {{ old('departments_id', $employee->departments_id) == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select class="form-select @error('skills') is-invalid @enderror" 
                        ...
                        @foreach($skills as $skill)
                            <option value="{{ $skill->id }}" 
                                {{ in_array($skill->id, old('skills', $employeeSkills)) ? 'selected' : '' }}>
                                {{ $skill->name }}
                            </option>
                        @endforeach
                    </select>
